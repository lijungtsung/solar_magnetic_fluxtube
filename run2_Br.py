...
def Br_ij_grid(i_float, j_float):
    z_tube = j_float * delta_z
    z_here_global = z_tube + iv.z_bottom
    r_here = i_float * delta_r
    return B_LT_final.Br_continuous(r_here, z_here_global)


def wrapping_calculate_Br_row_z(idx_z):
    ...
    for idx_r in range(N_grid_r):
        Br_row_z_arr += [Br_ij_grid(idx_r, idx_z)]
        
    Br_row_z_arr = np.array(Br_row_z_arr, dtype=np.float32)
    np.save(iv.B_CTM_output_dir+'temp_Br_rows/row%i.npy'%idx_z, Br_row_z_arr)    
...
if __name__ == '__main__':
    ...
    #### check whether temp_Br_rows folders exist or not ####
    directory_path = os.path.join(os.getcwd(), iv.B_CTM_output_dir, 'temp_Br_rows')
    ...
    with MPIPoolExecutor() as pool:
        pool.map(wrapping_calculate_Br_row_z, range(N_grid_z))
    
    
    #### join all the rows ####
    Br_all_rows = [None] * N_grid_z
    
    for idx_z in range(N_grid_z):
        temp_arr = np.load(iv.B_CTM_output_dir+'temp_Br_rows/row%i.npy'%idx_z)
        Br_all_rows[idx_z] = temp_arr.tolist()

    Br_all_rows = np.array(Br_all_rows, dtype=np.float32)
    np.save(iv.B_CTM_output_dir+'BrGrid_read.npy', Br_all_rows)
    del Br_all_rows
